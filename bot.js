...
// 🔹 DEBUG LINE: check if BOT_TOKEN is set
...
bot.on('message', async (msg) => {
    ...
    try {
        ...
        // Inform user about content type
        const contentType = urls[0].includes(".mp4") ? "Video" : "Photo";
        bot.sendMessage(chatId, `Downloading ${urls.length} ${contentType}${urls.length > 1 ? "s" : ""}...`);

        // Split media in groups of 10 for Telegram limits
        for (let i = 0; i < urls.length; i += 10) {
